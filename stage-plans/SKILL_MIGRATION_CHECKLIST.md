...
- [ ] Add Rust skill specs and deterministic first-pass executors.
- [ ] Keep registry integration out of this stage.
- [ ] Keep LLM/search/DB/platform side effects represented as deferred dependencies unless explicitly implemented.
...
- [ ] `ops_pricing_strategy` - 定价策略 - P0 - required: `cost`, `competitor_prices`
- [ ] `ops_channel_strategy` - 渠道策略 - P1 - required: `product_type`, `budget`
- [ ] `ops_inventory_planning` - 库存规划 - P0 - required: `avg_daily_demand`, `lead_time_days`
...
- [ ] `ops_smart_pricing` - 智能定价引擎 - P0 - required: `cost`, `current_price`
- [ ] `ops_inventory_optimizer` - 库存优化计算 - P0 - required: none
- [ ] `ops_ad_fatigue_detector` - 广告疲劳检测 - P0 - required: none
- [ ] `ops_abc_xyz_classifier` - ABC-XYZ库存分类 - P0 - required: none
...
- [ ] `accounting_cost_calc` - 成本核算 - P0 - required: `purchase_cost`, `selling_price`
- [ ] `accounting_profit_analysis` - 利润分析 - P0 - required: none
- [ ] `accounting_budget_plan` - 预算编制 - P0 - required: none
- [ ] `accounting_roi_calc` - ROI计算 - P0 - required: none
- [ ] `accounting_compliance_check` - 合规检查 - P0 - required: none
- [ ] `accounting_pl_statement` - 完整利润表 - P0 - required: none
- [ ] `accounting_break_even_calc` - 盈亏平衡分析 - P0 - required: none
- [ ] `accounting_cash_flow_forecast` - 现金流预测 - P1 - required: none
- [ ] `accounting_financial_narrative` - 财务健康诊断 - P1 - required: none
- [ ] `accounting_budget_vs_actual` - 预算vs实际分析 - P0 - required: none
- [ ] `accounting_gmv_waterfall` - GMV利润瀑布分析 - P0 - required: none
- [ ] `accounting_scenario_analysis` - 情景财务分析 - P0 - required: `base_gmv`

### Search (6)

- [ ] `search_trends` - 市场趋势搜索 - P2 - required: `query`
- [ ] `search_competitor` - 竞品信息搜索 - P2 - required: `query`
- [ ] `search_market_info` - 市场行情搜索 - P2 - required: `query`
- [ ] `search_product_reviews` - 用户评价搜索 - P2 - required: `query`
- [ ] `search_platform_policy` - 平台政策搜索 - P2 - required: `platform`
- [ ] `search_industry_benchmarks` - 行业基准搜索 - P2 - required: `industry`
...
- [ ] `creative_content_calendar` - 内容排期 - P0 - required: none
...
- [ ] `creative_title_ctr_scorer` - 标题CTR预测评分 - P0 - required: none
...
- [ ] `service_return_handler` - 退换货处理 - P0 - required: `reason`
- [ ] `service_query_product` - 查询商品信息 - P1 - required: none
- [ ] `service_nps_analyzer` - NPS客户满意度分析 - P0 - required: none
- [ ] `service_sentiment_analyzer` - 客服情感分析 - P0 - required: none
- [ ] `service_nps_driver_analysis` - NPS驱动因素分析 - P0 - required: none
...
- [ ] `design_material_spec` - 素材规范 - P0 - required: `usage`
...
- [ ] `engineering_sla_monitor` - SLA监控 - P0 - required: none

### Web (7)

- [ ] `web_seo_optimize` - SEO优化方案 - P1 - required: none
- [ ] `web_title_generator` - 标题生成 - P1 - required: none
- [ ] `web_page_conversion` - 页面转化优化 - P0 - required: `page_type`
...
- [ ] `web_title_seo_scorer` - 标题SEO评分 - P0 - required: none

### Coordination And Platform (11)

- [ ] `coordination_agent_handoff` - Agent交接 - P2 - required: `task_description`
